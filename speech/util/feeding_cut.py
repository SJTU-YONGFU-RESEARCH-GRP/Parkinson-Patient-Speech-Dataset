...
def masking(spectrum):
    num_holes = 2
    cat = "frequency"
    length = 2
    h = np.size(spectrum, 1)
    w = np.size(spectrum, 2)
    ...
    # mask = np.concat(mask_list, axis=2)
    # mask = np.uint8(mask)
    
    spectrum = spectrum * mask
    # spectrum = np.reshape(spectrum,[1,h,w])
    return spectrum


def mask(matrix):

    matrix = tf.py_func(masking, [matrix], tf.float32)
    return matrix
